...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def __call__(
        ...
        SSIM_list = []
        residual_list = []
        lamda = len(timesteps) * args.lamda
        ed = int(len(timesteps) * 0.7)
        eps = args.eps
        alpha = 1.0 / (ed-lamda)
        self.ssim_buffer = []
        self.first_fusion_flag = False
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if h*w<=tile_size*tile_size: # tiled latent input
                    ...
                    if len(SSIM_list) >= lamda or self.first_fusion_flag:
                        down_block_res_samples_2, mid_block_res_sample_2 = self.controlnet(
                            controlnet_latent_model_input,
                            t,
                            encoder_hidden_states=controlnet_prompt_embeds,
                            controlnet_cond=residual_list[1],
                            conditioning_scale=conditioning_scale,
                            guess_mode=guess_mode,
                            return_dict=False,
                            image_encoder_hidden_states=ram_encoder_hidden_states,
                        ...
                        noise_pred_2 = self.unet(
                            ...
                            encoder_hidden_states=prompt_embeds,
                            cross_attention_kwargs=cross_attention_kwargs,
                            down_block_additional_residuals=down_block_res_samples_2,
                            mid_block_additional_residual=mid_block_res_sample_2,
                            return_dict=False,
                            image_encoder_hidden_states=ram_encoder_hidden_states,
                        )[0]

                        if current_step <= len(timesteps) - ed:
                            c1 = 0
                        elif len(SSIM_list) >= lamda or self.first_fusion_flag:
                            c1 = 0
                            time_weight = torch.tensor(alpha * (len(timesteps) - lamda - current_step))
                            c1 = c1 * time_weight
                        noise_pred = noise_pred + c1 * (noise_pred_2 - noise_pred)
                else:
                ...
                if len(SSIM_list) >= lamda: #  or self.first_fusion_flag:
                    ed = int(len(timesteps) * 0.1)
                    import math
                    if current_step <= ed:
                        c2 = 0
                    else:
                        c2 = (current_step - ed) / (len(timesteps) - ed - lamda)
                        c2 = 2 * 0.8 * torch.clamp((torch.exp(torch.tensor(c2)) - 1) / (math.e - 1), min=0.0, max=1.0)
                        c2 = c2.item()

                    extra_step_kwargs['latents_pred'] = residual_list[0]
                    extra_step_kwargs['c2'] = c2
                # compute the previous noisy sample x_t -> x_t-1
                # latents = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]
                L = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)
                latents = L[0]# .to(prompt_embeds.dtype)
                latents_pred = L[1]# .to(prompt_embeds.dtype)

                if len(SSIM_list) < lamda or self.first_fusion_flag == False:
                    im_sr = self.vae.decode(latents_pred / self.vae.config.scaling_factor, return_dict=False)[0]
                    im_sr = (im_sr + 1) / 2
                    SR_image = im_sr
                    im_sr = torch.clamp(im_sr, min=0.0, max=1.0)
                    im_sr = im_sr.cpu().numpy().transpose(0, 2, 3, 1) * 255
                    im_lr = torch.clamp(image[0].unsqueeze(dim=0), min=0.0, max=1.0)
                    im_lr = im_lr.cpu().numpy().transpose(0, 2, 3, 1) * 255
                    SSIM = structural_similarity(im_sr[0], im_lr[0], data_range=255, channel_axis=2)
                    SSIM_list.append(SSIM)
                    while(len(self.ssim_buffer) > 0 and self.ssim_buffer[-1][0] < SSIM):
                        self.ssim_buffer.pop()
                    if(len(self.ssim_buffer) > 0 and (max(s[0] for s in self.ssim_buffer) - eps > SSIM)):
                        self.first_fusion_flag = True
                        self.lamda = len(timesteps) - current_step
                    self.ssim_buffer.append((SSIM, latents_pred))
                    # print(SSIM)
                    if max(SSIM_list) == SSIM and self.first_fusion_flag == False:
                        if len(residual_list) == 0:
                            residual_list.append(latents_pred)
                            residual_list.append(SR_image)
                        else:
                            residual_list[0] = latents_pred
                            residual_list[1] = SR_image

                    elif self.first_fusion_flag:
                        __latents = [item[1] for item in self.ssim_buffer]
                        residual_list[0] = torch.mean(torch.stack(__latents), dim=0)
                        im_sr = self.vae.decode(residual_list[0] / self.vae.config.scaling_factor, return_dict=False)[0]
                        im_sr = (im_sr + 1) / 2
                        residual_list[1] = im_sr
                        if(len(residual_list) < 3):
                            residual_list.append(len(self.ssim_buffer))
                        else:
                            residual_list[2] = len(self.ssim_buffer)


                elif self.first_fusion_flag:
                    alpha_ema = 0.9
                    residual_list[0] = residual_list[0] * alpha_ema + latents_pred * (1 - alpha_ema)
                    im_sr = self.vae.decode(residual_list[0] / self.vae.config.scaling_factor, return_dict=False)[0]
                    im_sr = (im_sr + 1) / 2
                    residual_list[1] = im_sr
                    residual_list[2] = residual_list[2] + 1
